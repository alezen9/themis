...
const nodes = [
  ...
  derived(p, "psi_LT_eff", "Effective psi for LT moment shape", ["moment_shape_LT", "psi_LT", "k_c"], { expression: "\\psi_{LT,eff}" }),
  ...
  derived(p, "torsion_add", "Torsion additive term in Mcr", ["torsionProduct", "torsionFactor"], {
  ...
  derived(p, "N_cr_y", "Elastic critical force y", ["N_cr_yProduct", "Lcr_y_sq"], { expression: "\\pi^2EI_y/L_{cr,y}^2" }),
  derived(p, "N_cr_zProduct", "Ncr,z numerator", ["piSq", "E", "Iz"], { expression: "\\pi^2EI_z" }),
  derived(p, "N_cr_z", "Elastic critical force z", ["N_cr_zProduct", "Lcr_z_sq"], { expression: "\\pi^2EI_z/L_{cr,z}^2" }),
  derived(p, "lambda_bar_y_sq", "Squared slenderness y", ["A", "fy", "N_cr_y"]),
  derived(p, "lambda_bar_y", "Slenderness y", ["lambda_bar_y_sq"]),
  derived(p, "lambda_bar_z_sq", "Squared slenderness z", ["A", "fy", "N_cr_z"]),
  derived(p, "lambda_bar_z", "Slenderness z", ["lambda_bar_z_sq"]),
  derived(p, "phi_y_alpha", "Alpha term in phi_y", ["alpha_y", "lambda_bar_y"]),
  derived(p, "phi_y_inner", "Inner sum in phi_y", ["phi_y_alpha", "lambda_bar_y_sq"]),
  derived(p, "phi_y", "Buckling parameter y", ["phi_y_inner"]),
  derived(p, "phi_y_sq", "Squared phi_y", ["phi_y"]),
  derived(p, "chi_y_radicand", "Radicand for chi_y", ["phi_y_sq", "lambda_bar_y_sq"]),
  derived(p, "chi_y_root", "Root for chi_y denominator", ["chi_y_radicand"]),
  derived(p, "chi_yFactor", "Denominator for chi_y", ["phi_y", "chi_y_root"]),
  derived(p, "chi_y_base", "Base chi_y", ["chi_yFactor"]),
  ...
  derived(p, "chi_zFactor", "Denominator for chi_z", ["phi_z", "chi_z_root"]),
  derived(p, "chi_z_base", "Base chi_z", ["chi_zFactor"]),
  ...
  derived(p, "phi_LT_alpha", "Alpha term in phi_LT", ["alpha_LT_eff", "lambda_LT", "lambda_LT_0"]),
  ...
  derived(p, "chi_LTFactor", "Denominator for chi_LT", ["phi_LT", "chi_LT_root"]),
  derived(p, "chi_LT_base", "Base chi_LT", ["chi_LTFactor"]),
  ...
  derived(p, "N_b_y_Rd", "Buckling resistance y", ["chi_y", "N_Rk", "gamma_M1"]),
  derived(p, "n_y", "Normalized axial load vs Nby,Rd", ["N_Ed", "N_b_y_Rd"]),
  derived(p, "lambda_bar_y_cap", "Capped slenderness y", ["lambda_bar_y"]),
  derived(p, "k_yy_branch1_term", "Branch factor term for k_yy", ["lambda_bar_y_cap"]),
  derived(p, "k_yy_branch1", "Branch 1 for k_yy", ["Cm_y", "k_yy_branch1_term", "n_y"]),
  derived(p, "k_yy_branch2", "Branch 2 for k_yy", ["Cm_y", "n_y"]),
  table(p, "k_yy", "Interaction factor k_yy", ["k_yy_branch1", "k_yy_branch2"], {
    source: "EC3-B.1",
  ...
  derived(p, "k_zz_aux", "Auxiliary k_zz", ["k_zz_branch1", "k_zz_branch2"]),
  table(p, "k_yz", "Interaction factor k_yz", ["k_zz_aux"], {
    source: "EC3-B.1",
    meta: { sectionRef: "Annex B", tableRef: "B.1" },
  }),
  derived(p, "bc_61_term1", "Eq.6.61 term 1", ["N_Ed", "chi_y", "N_Rk", "gamma_M1"]),
  derived(p, "bc_61_term2", "Eq.6.61 term 2", ["k_yy", "M_y_Ed", "chi_LT_mod", "M_y_Rk", "gamma_M1"]),
  derived(p, "bc_61_term3", "Eq.6.61 term 3", ["k_yz", "M_z_Ed", "M_z_Rk", "gamma_M1"]),
  check(p, "bc_61_m2_check", "Beam-column Eq.6.61 Method 2", ["bc_61_term1", "bc_61_term2", "bc_61_term3"], {
  ...
  evaluate: {
    k_c: ({ moment_shape_LT, psi_LT, support_condition_LT, load_application_LT }) =>
      getKcFromEc3Table66({
        ...
        contextLabel: "beam-column-61-m2:k_c",
      }),
    ...
    Lcr_y_sq: ({ Lcr_y }) => {
      if (Lcr_y <= 0) throwInvalidInput("beam-column-61-m2: Lcr_y must be > 0");
      return Lcr_y ** 2;
    },
    Lcr_z_sq: ({ Lcr_z }) => {
      if (Lcr_z <= 0) throwInvalidInput("beam-column-61-m2: Lcr_z must be > 0");
      return Lcr_z ** 2;
    },
    Lcr_LT_sq: ({ Lcr_LT }) => {
      if (Lcr_LT <= 0) throwInvalidInput("beam-column-61-m2: Lcr_LT must be > 0");
      return Lcr_LT ** 2;
    },
    psi_y_eff: ({ moment_shape_y, support_condition_y, psi_y }): number => {
      ...
      if (moment_shape_y === "linear") {
        if (typeof psi_y !== "number" || Number.isNaN(psi_y)) {
          return throwEc3VerificationError({
            type: "MISSING_INPUT",
            message: "beam-column-61-m2: psi_y is required when moment_shape_y is linear",
      ...
      return throwInvalidInput("beam-column-61-m2: unsupported moment_shape_y");
    },
    psi_z_eff: ({ moment_shape_z, support_condition_z, psi_z }): number => {
      ...
      if (moment_shape_z === "linear") {
        if (typeof psi_z !== "number" || Number.isNaN(psi_z)) {
          return throwEc3VerificationError({
            type: "MISSING_INPUT",
            message: "beam-column-61-m2: psi_z is required when moment_shape_z is linear",
      ...
      return throwInvalidInput("beam-column-61-m2: unsupported moment_shape_z");
    },
    ...
    torsion_ratio: ({ Iw, Iz }) => {
      if (Iz <= 0) throwInvalidInput("beam-column-61-m2: Iz must be > 0");
      return Iw / Iz;
    },
    ...
    torsion_root: ({ torsion_sum }) => {
      if (torsion_sum <= 0) throwInvalidInput("beam-column-61-m2: invalid torsion term");
    ...
    M_cr: ({ section_shape, section_class, Iz, It, Iw, M_cr_prefactor, torsion_root }) => {
      if (section_class === 4) {
        throwEc3VerificationError({
          ...
          message: "beam-column-61-m2: class 4 sections are out of scope",
      ...
      if (section_shape !== "I") {
        throwEc3VerificationError({
          ...
          message: "beam-column-61-m2: SN003b M_cr is implemented only for I/H sections",
          details: { section_shape, sectionRef: "6.3.3" },
        });
      }
      if (Iz <= 0 || It <= 0 || Iw <= 0) throwInvalidInput("beam-column-61-m2: Iz, It and Iw must be > 0");
    ...
    lambda_bar_y_sq: ({ A, fy, N_cr_y }) => (A * fy) / N_cr_y,
    ...
    phi_y_alpha: ({ alpha_y, lambda_bar_y }) => alpha_y * (lambda_bar_y - 0.2),
    phi_y_inner: ({ phi_y_alpha, lambda_bar_y_sq }) => 1 + phi_y_alpha + lambda_bar_y_sq,
    phi_y: ({ phi_y_inner }) => 0.5 * phi_y_inner,
    phi_y_sq: ({ phi_y }) => phi_y ** 2,
    chi_y_radicand: ({ phi_y_sq, lambda_bar_y_sq }) => phi_y_sq - lambda_bar_y_sq,
    ...
    lambda_LT_sq: ({ Wpl_y, fy, M_cr }) => {
      if (M_cr <= 0) throwInvalidInput("beam-column-61-m2: M_cr must be > 0");
    ...
    phi_LT_alpha: ({ alpha_LT_eff, lambda_LT, lambda_LT_0 }) => alpha_LT_eff * (lambda_LT - lambda_LT_0),
    ...
    f_LT: ({ coefficient_f_method, lambda_LT, k_c }) =>
      (coefficient_f_method ?? "default-equation") === "force-1.0"
        ? 1
        : getFReductionEq658(lambda_LT, k_c),
    chi_LT_mod: ({ chi_LT, f_LT }) => {
      if (f_LT <= 0) throwInvalidInput("beam-column-61-m2: f_LT must be > 0");
      return Math.min(1, chi_LT / f_LT);
    },
    N_b_y_Rd: ({ chi_y, N_Rk, gamma_M1 }) => (chi_y * N_Rk) / gamma_M1,
    n_y: ({ N_Ed, N_b_y_Rd, torsional_deformations, interaction_factor_method }) => {
      if (N_b_y_Rd <= 0) throwInvalidInput("beam-column-61-m2: NbyRd must be > 0");
      if ((torsional_deformations ?? "yes") !== "yes") {
        throwNotApplicableLoadCase("beam-column-61-m2: torsional deformations are disabled", {
          torsional_deformations,
          sectionRef: "6.3.3",
        });
      }
      if ((interaction_factor_method ?? "both") === "method1") {
        throwNotApplicableLoadCase("beam-column-61-m2: interaction factor method set to method1", {
          interaction_factor_method,
          sectionRef: "6.3.3(5)",
        });
      }
      if (N_Ed > 0) {
        throwNotApplicableLoadCase("beam-column-61-m2: check is only applicable for compression (N_Ed < 0)", {
          N_Ed,
          sectionRef: "6.3.3",
        });
      }
    ...
    lambda_bar_y_cap: ({ lambda_bar_y }) => Math.min(lambda_bar_y, 1),
    k_yy_branch1_term: ({ lambda_bar_y_cap }) => lambda_bar_y_cap - 0.2,
    k_yy_branch1: ({ Cm_y, k_yy_branch1_term, n_y }) => Cm_y * (1 + k_yy_branch1_term * n_y),
    k_yy_branch2: ({ Cm_y, n_y }) => Cm_y * (1 + 0.8 * n_y),
    k_yy: ({ k_yy_branch1, k_yy_branch2 }) => Math.min(k_yy_branch1, k_yy_branch2),
    N_b_z_Rd: ({ chi_z, N_Rk, gamma_M1 }) => (chi_z * N_Rk) / gamma_M1,
    n_z: ({ N_Ed, N_b_z_Rd }) => {
      if (N_b_z_Rd <= 0) throwInvalidInput("beam-column-61-m2: NbzRd must be > 0");
      if (N_Ed > 0) {
        throwNotApplicableLoadCase("beam-column-61-m2: check is only applicable for compression (N_Ed < 0)", {
          N_Ed,
          sectionRef: "6.3.3",
    ...
    k_zz_aux: ({ k_zz_branch1, k_zz_branch2 }) => Math.min(k_zz_branch1, k_zz_branch2),
    k_yz: ({ k_zz_aux }) => 0.6 * k_zz_aux,
    ...
    bc_61_term2: ({ k_yy, M_y_Ed, chi_LT_mod, M_y_Rk, gamma_M1 }) => k_yy * Math.abs(M_y_Ed) / ((chi_LT_mod * M_y_Rk) / gamma_M1),
    bc_61_term3: ({ k_yz, M_z_Ed, M_z_Rk, gamma_M1 }) => k_yz * Math.abs(M_z_Ed) / (M_z_Rk / gamma_M1),
    bc_61_m2_check: ({ bc_61_term1, bc_61_term2, bc_61_term3 }) => bc_61_term1 + bc_61_term2 + bc_61_term3,
  },
};
