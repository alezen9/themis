...
describe("check-01 tension", () => {
  it("computes plastic resistance and utilization from EC3 formula inputs", () => {
    ...
    const expectedRatio = baselineInputs.N_Ed / expectedNplRd;

    expect(result.cache.N_pl_Rd).toBeCloseTo(expectedNplRd, 12);
    expect(result.ratio).toBeCloseTo(expectedRatio, 12);
    expect(result.passed).toBe(true);
  });

  it("increases utilization when tensile force increases", () => {
  ...
  it("passes at ratio = 1", () => {
    const nplRd =
      ...
      customAnnex.coefficients.gamma_M0;
    const result = evaluate(check, {
      inputs: { ...baselineInputs, N_Ed: nplRd },
      annex: customAnnex,
    });
  ...
  it("fails when ratio is above 1", () => {
    const nplRd =
      ...
      customAnnex.coefficients.gamma_M0;
    const result = evaluate(check, {
      inputs: { ...baselineInputs, N_Ed: 1.01 * nplRd },
      annex: customAnnex,
    });
...
describe("check-01 reference scenarios", () => {
  it("returns expected resistance and utilization for CHS class-1 reference values", () => {
    ...
    expect(result.cache.N_pl_Rd).toBeCloseTo(152_190.476, 0);
  ...
  it("returns expected resistance and utilization for RHS class-1 reference values", () => {
    const result = evaluate(check, {
      inputs: { A: 1915, fy: 235, N_Ed: 30_000 },
      annex: parityAnnex,
    });

    expect(result.cache.N_pl_Rd).toBeCloseTo(428_595.238, 0);
  ...
  it("returns expected resistance and utilization for IPE300 class-2 reference values", () => {
    const result = evaluate(check, {
      inputs: { A: 5381, fy: 235, N_Ed: 3_000_000 },
      annex: parityAnnex,
    });

    expect(result.cache.N_pl_Rd).toBeCloseTo(1_204_319.048, 0);
  ...
  it("returns expected resistance and utilization for IPE600 class-3 reference values", () => {
    ...
    expect(result.cache.N_pl_Rd).toBeCloseTo(3_490_980.952, 0);
    expect(result.ratio).toBeCloseTo(0.859, 3);
    expect(result.passed).toBe(true);
  });

  it("throws not-applicable-load-case for compression-sign reference values", () => {
    try {
      evaluate(check, {
        inputs: { A: 5381, fy: 235, N_Ed: -1_100_000 },
        annex: parityAnnex,
      });
      throw new Error("expected evaluation to fail");
    } catch (error) {
