...
import type { Nodes } from "./ulsBeamColumn61M2-nodes";
...
export const evaluate = defineEvaluators<Nodes, Ec3EvaluatorInputs>({
  ...
  M_cr: ({ L, k_LT, Iz, It, Iw, C1, pi, E, G, section_shape }) => {
    ...
    if (Iz <= 0 || It <= 0 || Iw <= 0)
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message: "beam-column-61-m2: Iz, It and Iw must be > 0",
      });
    const lcrLT = L * k_LT;
    if (!Number.isFinite(lcrLT) || lcrLT <= 0)
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message: "beam-column-61-m2: L*k_LT must be > 0",
      });
    if (C1 <= 0)
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
  ...
  N_cr_y: ({ pi, E, Iy, L, k_y }) => {
    ...
    if (!Number.isFinite(denominator) || denominator <= 0) {
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message:
          "beam-column-61-m2: denominator (L*k_y)^2 must be > 0 (division by zero)",
      });
    }
  ...
  N_cr_z: ({ pi, E, Iz, L, k_z }) => {
    ...
    if (!Number.isFinite(denominator) || denominator <= 0) {
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message:
          "beam-column-61-m2: denominator (L*k_z)^2 must be > 0 (division by zero)",
      });
    }
  ...
  bc_61_term2: ({ k_yy, M_y_Ed, chi_LT_mod, M_y_Rk, gamma_M1 }) =>
    (k_yy * Math.abs(M_y_Ed)) / ((chi_LT_mod * M_y_Rk) / gamma_M1),
  bc_61_term3: ({ k_yz, M_z_Ed, M_z_Rk, gamma_M1 }) =>
    (k_yz * Math.abs(M_z_Ed)) / (M_z_Rk / gamma_M1),
