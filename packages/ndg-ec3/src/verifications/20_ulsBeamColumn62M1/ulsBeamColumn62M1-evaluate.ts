...
export const evaluate = defineEvaluators<Nodes, Ec3EvaluatorInputs>({
  ...
  N_cr_y: ({ pi, E, Iy, L, k_y }) => {
    ...
    if (!Number.isFinite(denominator) || denominator <= 0) {
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message:
          "beam-column-62-m1: denominator (L*k_y)^2 must be > 0 (division by zero)",
      });
    }
  ...
  N_cr_z: ({ pi, E, Iz, L, k_z }) => {
    ...
    if (!Number.isFinite(denominator) || denominator <= 0) {
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message:
          "beam-column-62-m1: denominator (L*k_z)^2 must be > 0 (division by zero)",
      });
    }
  ...
  ip2: ({ Iy, Iz, A }) => {
    if (A <= 0)
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message: "beam-column-62-m1: A must be > 0",
      });
    const value = (Iy + Iz) / A;
    if (value <= 0)
      throw new Ec3VerificationError({
  ...
  N_cr_T: ({ Iy, Iz, It, Iw, G, ncr_t_warp, ip2 }) => {
    if (Iy <= 0 || Iz <= 0 || It <= 0 || Iw <= 0) {
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message: "beam-column-62-m1: Iy, Iz, It and Iw must be > 0",
      });
    }
  ...
  M_cr: ({ L, k_LT, Iz, It, Iw, C1, pi, E, G }) => {
    ...
    if (!Number.isFinite(lcrLT) || lcrLT <= 0)
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message: "beam-column-62-m1: L*k_LT must be > 0",
      });
    if (Iz <= 0 || It <= 0 || Iw <= 0)
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message: "beam-column-62-m1: Iz, It and Iw must be > 0",
      });
    if (C1 <= 0)
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
  ...
  lambda_bar_0: ({ Wpl_y, fy, C1, M_cr }) => {
    if (C1 <= 0)
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message: "beam-column-62-m1: C1 must be > 0",
      });
    return Math.sqrt((Wpl_y * fy * C1) / M_cr);
  },
  ...
  ncr_y_ratio: ({ abs_N_Ed, N_cr_y }) => {
    if (N_cr_y <= 0)
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message: "beam-column-62-m1: N_cr_y must be > 0",
      });
  ...
  ncr_z_ratio: ({ abs_N_Ed, N_cr_z }) => {
    if (N_cr_z <= 0)
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message: "beam-column-62-m1: N_cr_z must be > 0",
      });
  ...
  ncr_t_ratio: ({ abs_N_Ed, N_cr_T }) => {
    if (N_cr_T <= 0)
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message: "beam-column-62-m1: N_cr_T must be > 0",
      });
  ...
  ncr_tf_ratio: ({ abs_N_Ed, N_cr_TF }) => {
    if (N_cr_TF <= 0)
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message: "beam-column-62-m1: N_cr_TF must be > 0",
      });
    return abs_N_Ed / N_cr_TF;
  },
  cm_branch_limit: ({ ncr_z_ratio, ncr_tf_ratio }) =>
    0.4 * (1 - ncr_z_ratio) * (1 - ncr_tf_ratio),
  cm_branch_active: ({ lambda_bar_0, C1, cm_branch_limit }) =>
    lambda_bar_0 > 0.2 && C1 < cm_branch_limit ? 1 : 0,
  cm_amp: ({ eta_y, a_LT }) => {
    const numerator = eta_y * a_LT;
  ...
  Cm_LT: ({ cm_branch_active, Cm_y_aug, a_LT, cmReserve }) => {
    ...
    const value = (Cm_y_aug ** 2 * a_LT) / cmReserve;
  ...
  chi_LT: ({ Wpl_y, fy, M_cr, alpha_LT_eff, lambda_LT_0, beta_LT }) => {
    if (M_cr <= 0)
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message: "beam-column-62-m1: M_cr must be > 0",
      });
  ...
  f_LT: (args) => {
    const { coefficient_f_method, Wpl_y, fy, M_cr, k_c } = args;
  ...
  k_zy: ({ abs_N_Ed, Cm_y, Cm_LT, k_yyReserve, C_zy, wy, wz, k_yy }) => {
    if (abs_N_Ed <= 1e-12) {
      return (
        (Cm_y * Cm_LT * (1 / C_zy) * 0.6 * Math.sqrt(wy / wz)) / k_yyReserve
      );
    }
    return 0.6 * k_yy;
  },
  ...
  bc_62_term2: ({ k_zy, abs_M_y_Ed, chi_LT_mod, M_y_Rk, gamma_M1 }) => {
    ...
    return k_zy * (abs_M_y_Ed / denominator);
  },
  bc_62_term3: ({ k_zz, abs_M_z_Ed, M_z_Rk, gamma_M1 }) => {
    ...
    return k_zz * (abs_M_z_Ed / denominator);
  },
