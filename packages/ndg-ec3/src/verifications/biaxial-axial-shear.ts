...
const nodes = [
  ...
  derived(p, "V_plFactor", "Common denominator for V_pl,Rd", ["sqrt3", "gamma_M0"]),
  formula(p, "V_pl_z_Rd", "Plastic shear resistance z", ["V_pl_zProduct", "V_plFactor"], {
    symbol: "V_{pl,z,Rd}",
  ...
  formula(p, "V_pl_y_Rd", "Plastic shear resistance y", ["V_pl_yProduct", "V_plFactor"], {
    symbol: "V_{pl,y,Rd}",
  ...
  formula(p, "M_y_V_Rd", "Reduced plastic moment y-y allowing for shear", ["M_y_VProduct", "gamma_M0"], {
    symbol: "M_{y,V,Rd}",
    expression: "\\frac{\\left(W_{pl,y} - \\rho_z \\dfrac{A_{v,z}^2}{4t_w}\\right) f_y}{\\gamma_{M0}}",
    unit: "N·mm",
    meta: { sectionRef: "6.2.8", formulaRef: "(6.30)" },
  }),
  derived(p, "axial_yProduct", "Numerator axial factor y", ["n"]),
  derived(p, "axial_yFactor", "Denominator axial factor y", ["a_w"]),
  derived(p, "axial_y_ratio", "Raw axial factor y", ["axial_yProduct", "axial_yFactor"]),
  ...
  derived(p, "M_z_V_Rd", "Reduced plastic moment z-z allowing for shear", ["class_guard", "M_z_VProduct", "gamma_M0"], {
    symbol: "M_{z,V,Rd}",
...
export const ulsBiaxialAxialShear: VerificationDefinition<typeof nodes> = {
  ...
  evaluate: {
    ...
    V_plFactor: ({ sqrt3, gamma_M0 }) => sqrt3 * gamma_M0,
    V_pl_z_Rd: ({ V_pl_zProduct, V_plFactor }) => V_pl_zProduct / V_plFactor,
    V_pl_yProduct: ({ Av_y, fy }) => Av_y * fy,
    V_pl_y_Rd: ({ V_pl_yProduct, V_plFactor }) => V_pl_yProduct / V_plFactor,
    ...
    a_w_raw: ({ section_shape, A, Av_y, Av_z }) => {
      if (A <= 0) throwInvalidInput("biaxial-axial-shear: A must be > 0");
    ...
    M_y_V_Rd: ({ M_y_VProduct, gamma_M0 }) => M_y_VProduct / gamma_M0,
