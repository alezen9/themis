...
const nodes = [
  ...
  derived(p, "k_zy_cmReserve", "Cm_LT denominator term for k_zy", ["Cm_LT"]),
  derived(p, "k_zy_lambda_ratio", "Lambda/Cm term for k_zy", ["lambda_bar_z", "k_zy_cmReserve"]),
  ...
  derived(p, "k_zy_high_a", "High branch candidate A", ["k_zy_high_lambda_cap", "k_zy_cmReserve", "n_z"]),
  derived(p, "k_zy_high_b", "High branch candidate B", ["k_zy_cmReserve", "n_z"]),
...
export const ulsBeamColumn62M2: VerificationDefinition<typeof nodes> = {
  ...
  evaluate: {
    ...
    psi_y_eff: ({ moment_shape_y, support_condition_y, psi_y }): number => {
      ...
      return throwInvalidInput("beam-column-62-m2: unsupported moment_shape_y");
    },
    psi_z_eff: ({ moment_shape_z, support_condition_z, psi_z }): number => {
      ...
      return throwInvalidInput("beam-column-62-m2: unsupported moment_shape_z");
    },
    ...
    n_z: ({ N_Ed, N_b_z_Rd, torsional_deformations, interaction_factor_method }) => {
      if (N_b_z_Rd <= 0) throwInvalidInput("beam-column-62-m2: NbzRd must be > 0");
      if ((torsional_deformations ?? "yes") !== "yes") {
        throwNotApplicableLoadCase("beam-column-62-m2: torsional deformations are disabled", {
          torsional_deformations,
          sectionRef: "6.3.3",
        });
      }
      if ((interaction_factor_method ?? "both") === "method1") {
        throwNotApplicableLoadCase("beam-column-62-m2: interaction factor method set to method1", {
    ...
    k_zy_high_a: ({ k_zy_high_lambda_cap, k_zy_cmReserve, n_z }) => 1 - (0.1 * k_zy_high_lambda_cap / k_zy_cmReserve) * n_z,
    k_zy_high_b: ({ k_zy_cmReserve, n_z }) => 1 - (0.1 / k_zy_cmReserve) * n_z,
