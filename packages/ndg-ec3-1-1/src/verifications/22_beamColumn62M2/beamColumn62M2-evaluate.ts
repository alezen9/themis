...
export const evaluate = defineEvaluators(nodes, {
  ...
  M_cr_Nmm: ({
    ...
    if (Iz_mm4 <= 0 || It_mm4 <= 0 || Iw_mm6 <= 0)
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message: "beam-column-62-m2: Iz_mm4, It_mm4 and Iw_mm6 must be > 0",
      });
    const lcrLT = L_mm * k_LT;
    if (!Number.isFinite(lcrLT) || lcrLT <= 0)
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message: "beam-column-62-m2: L_mm*k_LT must be > 0",
      });
    if (C1 <= 0)
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
  ...
  N_cr_y_N: ({ pi, E_MPa, Iy_mm4, L_mm, k_y }) => {
    ...
    if (!Number.isFinite(denominator) || denominator <= 0) {
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message:
          "beam-column-62-m2: denominator (L_mm*k_y)^2 must be > 0 (division by zero)",
      });
    }
  ...
  N_cr_z_N: ({ pi, E_MPa, Iz_mm4, L_mm, k_z }) => {
    ...
    if (!Number.isFinite(denominator) || denominator <= 0) {
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message:
          "beam-column-62-m2: denominator (L_mm*k_z)^2 must be > 0 (division by zero)",
      });
    }
  ...
  phi_y: ({ alpha_y, A_mm2, fy_MPa, N_cr_y_N }) =>
    0.5 *
    (1 +
      alpha_y * (Math.sqrt((A_mm2 * fy_MPa) / N_cr_y_N) - 0.2) +
      (A_mm2 * fy_MPa) / N_cr_y_N),
  chi_y: ({ phi_y, A_mm2, fy_MPa, N_cr_y_N }) => {
    const radicand = phi_y ** 2 - (A_mm2 * fy_MPa) / N_cr_y_N;
  ...
  lambda_LT: ({ Wpl_y_mm3, Wel_y_mm3, fy_MPa, M_cr_Nmm, section_class }) => {
    if (M_cr_Nmm <= 0)
      throw new Ec3VerificationError({
        type: "invalid-input-domain",
        message: "beam-column-62-m2: M_cr_Nmm must be > 0",
      });
  ...
  n_y: ({ N_Ed_N, N_b_y_Rd_N }) => {
    ...
    if (N_Ed_N > 0) {
      throw new Ec3VerificationError({
        ...
        message:
          "beam-column-62-m2: check is only applicable for compression (N_Ed_N < 0)",
  ...
  bc_62_term1: ({ N_Ed_N, chi_z, N_Rk_N, gamma_M1 }) => {
    if (N_Ed_N > 0) {
      throw new Ec3VerificationError({
        type: "not-applicable-load-case",
        message:
    ...
    return -N_Ed_N / ((chi_z * N_Rk_N) / gamma_M1);
  },
  bc_62_term2: ({ k_zy, M_y_Ed_Nmm, chi_LT_mod, M_y_Rk_Nmm, gamma_M1 }) =>
    (k_zy * Math.abs(M_y_Ed_Nmm)) / ((chi_LT_mod * M_y_Rk_Nmm) / gamma_M1),
  bc_62_term3: ({ k_zz, M_z_Ed_Nmm, M_z_Rk_Nmm, gamma_M1 }) =>
    (k_zz * Math.abs(M_z_Ed_Nmm)) / (M_z_Rk_Nmm / gamma_M1),
