import { defineNodes } from "@ndg/ndg-core";

export const nodes = defineNodes([
  {
    id: "ratio",
    type: "check",
    key: "ratio",
    valueType: { type: "number" },
    name: "Bending and shear resistance check about y-y",
    verificationExpression:
      "\\frac{\\left|M_{y,Ed}\\right|}{M_{y,V,Rd}} \\leq 1.0",
    meta: { sectionRef: "6.2.8", verificationRef: "(6.29)" },
    children: [{ nodeId: "M_y_Ed_Nmm" }, { nodeId: "M_y_V_Rd_Nmm" }],
  },
  {
    id: "M_y_Ed_Nmm",
    type: "user-input",
    key: "M_y_Ed_Nmm",
    valueType: { type: "number" },
    name: "Design bending moment about y-y",
    symbol: "M_{y,Ed}",
    unit: "\\mathrm{N\\cdot mm}",
    children: [],
  },
  {
    id: "M_y_V_Rd_Nmm",
    type: "derived",
    key: "M_y_V_Rd_Nmm",
    valueType: { type: "number" },
    name: "Reduced bending resistance about y-y under shear",
    symbol: "M_{y,V,Rd}",
    unit: "\\mathrm{N\\cdot mm}",
    meta: { sectionRef: "6.2.8", formulaRef: "(6.30)" },
    children: [
      { nodeId: "M_y_V_Rd_i_Nmm", when: { eq: ["shape", { value: "I" }] } },
      {
        nodeId: "M_y_V_Rd_rhs_chs_Nmm",
        when: {
          or: [
            { eq: ["shape", { value: "RHS" }] },
            { eq: ["shape", { value: "CHS" }] },
          ],
        },
      },
    ],
  },
  {
    id: "M_y_V_Rd_i_Nmm",
    type: "formula",
    key: "M_y_V_Rd_i_Nmm",
    valueType: { type: "number" },
    name: "I-section reduced bending resistance about y-y under shear",
    symbol: "M_{y,V,Rd,i}",
    expression:
      "\\frac{\\left(W_{y,res} - \\rho \\dfrac{A_w_mm2^2}{4t_w}\\right) \\cdot f_y}{\\gamma_{M0}} \\leq M_{c,y,Rd}",
    unit: "\\mathrm{N\\cdot mm}",
    meta: { sectionRef: "6.2.8", formulaRef: "(6.30)" },
    children: [
      { nodeId: "W_y_res_mm3" },
      { nodeId: "rho_z" },
      { nodeId: "A_w_mm2" },
      { nodeId: "tw_mm" },
      { nodeId: "fy_MPa" },
      { nodeId: "gamma_M0" },
    ],
  },
  {
    id: "M_y_V_Rd_rhs_chs_Nmm",
    type: "derived",
    key: "M_y_V_Rd_rhs_chs_Nmm",
    valueType: { type: "number" },
    name: "RHS/CHS reduced bending resistance about y-y under shear",
    symbol: "M_{y,V,Rd,RHS/CHS}",
    expression:
      "\\frac{\\left(W_{y,res}(1-\\rho)\\right) \\cdot f_y}{\\gamma_{M0}} \\leq M_{c,y,Rd}",
    unit: "\\mathrm{N\\cdot mm}",
    meta: { sectionRef: "6.2.8", paragraphRef: "(3)" },
    children: [
      { nodeId: "W_y_res_mm3" },
      { nodeId: "rho_z" },
      { nodeId: "fy_MPa" },
      { nodeId: "gamma_M0" },
    ],
  },
  {
    id: "W_y_res_mm3",
    type: "derived",
    key: "W_y_res_mm3",
    valueType: { type: "number" },
    name: "Class-dependent resistance modulus about y-y",
    symbol: "W_{y,res}",
    unit: "\\mathrm{mm^{3}}",
    children: [
      {
        nodeId: "Wpl_y_mm3",
        when: {
          or: [
            { eq: ["section_class", { value: 1 }] },
            { eq: ["section_class", { value: 2 }] },
          ],
        },
      },
      { nodeId: "Wel_y_mm3", when: { eq: ["section_class", { value: 3 }] } },
    ],
  },
  {
    id: "Wpl_y_mm3",
    type: "user-input",
    key: "Wpl_y_mm3",
    valueType: { type: "number" },
    name: "Plastic section modulus about y-y",
    symbol: "W_{pl,y}",
    unit: "\\mathrm{mm^{3}}",
    children: [],
  },
  {
    id: "Wel_y_mm3",
    type: "user-input",
    key: "Wel_y_mm3",
    valueType: { type: "number" },
    name: "Elastic section modulus about y-y",
    symbol: "W_{el,y}",
    unit: "\\mathrm{mm^{3}}",
    children: [],
  },
  {
    id: "rho_z",
    type: "derived",
    key: "rho_z",
    valueType: { type: "number" },
    name: "Shear reduction factor",
    symbol: "\\rho",
    children: [
      { nodeId: "rho_z_1", when: { lte: ["u_z", { value: 0.5 }] } },
      { nodeId: "rho_z_2", when: { gt: ["u_z", { value: 0.5 }] } },
    ],
  },
  {
    id: "rho_z_1",
    type: "derived",
    key: "rho_z_1",
    valueType: { type: "number" },
    name: "Shear reduction factor for low-shear branch",
    symbol: "\\rho_{z,1}",
    expression: "0",
    children: [],
  },
  {
    id: "rho_z_2",
    type: "derived",
    key: "rho_z_2",
    valueType: { type: "number" },
    name: "Shear reduction factor for high-shear branch",
    symbol: "\\rho_{z,2}",
    expression: "\\left(2u_z - 1\\right)^2",
    children: [{ nodeId: "u_z" }],
  },
  {
    id: "V_z_Ed_N",
    type: "user-input",
    key: "V_z_Ed_N",
    valueType: { type: "number" },
    name: "Design shear force about z-z",
    symbol: "V_{z,Ed}",
    unit: "\\mathrm{N}",
    children: [],
  },
  {
    id: "V_pl_z_Rd_N",
    type: "formula",
    key: "V_pl_z_Rd_N",
    valueType: { type: "number" },
    name: "Plastic shear resistance about z-z",
    symbol: "V_{pl,z,Rd}",
    expression: "\\dfrac{A_{v,z} \\cdot f_y}{\\sqrt{3}\\,\\gamma_{M0}}",
    unit: "\\mathrm{N}",
    meta: { sectionRef: "6.2.6", formulaRef: "(6.18)" },
    children: [
      { nodeId: "Av_z_mm2" },
      { nodeId: "fy_MPa" },
      { nodeId: "gamma_M0" },
    ],
  },
  {
    id: "u_z",
    type: "derived",
    key: "u_z",
    valueType: { type: "number" },
    name: "Shear utilization about z-z",
    symbol: "u_z",
    expression: "\\left|V_{z,Ed}\\right| / V_{pl,z,Rd}",
    children: [{ nodeId: "V_z_Ed_N" }, { nodeId: "V_pl_z_Rd_N" }],
  },
  {
    id: "Av_z_mm2",
    type: "user-input",
    key: "Av_z_mm2",
    valueType: { type: "number" },
    name: "Shear area about z-z",
    symbol: "A_{v,z}",
    unit: "\\mathrm{mm^{2}}",
    children: [],
  },
  {
    id: "hw_mm",
    type: "user-input",
    key: "hw_mm",
    valueType: { type: "number" },
    name: "Web depth",
    symbol: "h_w",
    unit: "\\mathrm{mm}",
    children: [],
  },
  {
    id: "A_w_mm2",
    type: "derived",
    key: "A_w_mm2",
    valueType: { type: "number" },
    name: "Web area",
    symbol: "A_w_mm2",
    expression: "h_w \\cdot t_w",
    unit: "\\mathrm{mm^{2}}",
    children: [{ nodeId: "hw_mm" }, { nodeId: "tw_mm" }],
  },
  {
    id: "tw_mm",
    type: "user-input",
    key: "tw_mm",
    valueType: { type: "number" },
    name: "Web thickness",
    symbol: "t_w",
    unit: "\\mathrm{mm}",
    children: [],
  },
  {
    id: "fy_MPa",
    type: "user-input",
    key: "fy_MPa",
    valueType: { type: "number" },
    name: "Yield strength",
    symbol: "f_y",
    unit: "\\mathrm{MPa}",
    children: [],
  },
  {
    id: "gamma_M0",
    type: "coefficient",
    key: "gamma_M0",
    valueType: { type: "number" },
    name: "Partial safety factor",
    symbol: "\\gamma_{M0}",
    meta: { sectionRef: "6.1" },
    children: [],
  },
]);

export type Nodes = typeof nodes;
