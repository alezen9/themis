...
const VerboseVerification = ({ result }: { result: VerificationRow }) => {
  ...
  return (
    <div className="mb-6 border rounded-lg overflow-hidden">
      ...
      <div
        className={`px-4 py-2 flex items-center justify-between ${headerBg} border-b`}
      >
          ...
          <div>
            ...
            <div className="bg-gray-50 rounded px-3 py-2 space-y-1.5">
              {checkEntry?.evaluatorInputs &&
                Object.keys(checkEntry.evaluatorInputs).length > 0 && (
                  <div className="flex flex-wrap gap-x-6 gap-y-0.5 pb-1.5 border-b border-gray-200">
                    {Object.entries(checkEntry.evaluatorInputs).map(
                      ([k, v]) => {
                        const sub = traceMap.get(k);
              ...
              <p className="font-mono text-sm">
                ...
                {" = "}
                <span className="font-medium">
                  {data?.ratio.toFixed(4)}
                </span>{" "}
                <span
                  className={data?.passed ? "text-green-700" : "text-red-600"}
                >
                  {data?.passed ? "\u2264" : ">"} 1.0
                </span>
                {"  "}
...
export const Ec3Report = ({ results, mode }: Ec3ReportProps) => {
  ...
  return (
    <div>
      ...
      {mode === "verbose"
        ? results.map((row) => (
            <VerboseVerification key={row.checkId} result={row} />
          ))
        : results.map((r, i) => (
            <SummaryVerification key={r.checkId} result={r} index={i} />
          ))}
    </div>
  );
};
